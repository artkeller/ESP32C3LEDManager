...
### The Problem:
Since a `NeoPixel` (WS2812) uses a single data line – in our case GPIO8 – for the transmission of color information, switching directly between these two control methods on the same pin can lead to unexpected behavior. Our extensive testing has revealed a critical issue: When the blue LED is controlled with PWM at a very high duty cycle (e.g., values above 240 at 8-bit resolution), the NeoPixel LED may interpret this as a faulty data signal and abruptly switch to full white brightness (RGB:255,255,255). This means that the correct timing of the data pulses is crucial for the correct operation of this `NeoPixel`, and this phenomenon is a direct consequence of an excessive duty cycle of the PWM signal that triggers the `NeoPixel`.
