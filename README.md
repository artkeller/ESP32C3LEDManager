...
The blue LED is typically controlled using simple digital HIGH/LOW signals or Pulse Width Modulation (PWM) via the ESP32's LEDC peripheral. On this specific board, the blue LED is not inverted (LOW = OFF, HIGH = ON). This LED of the **"normal" ESP32C3 SuoerMini** boards is inverted; it is sinked, not sourced (LOW = ON, HIGH = OFF).
